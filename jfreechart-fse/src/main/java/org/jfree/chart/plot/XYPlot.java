            ...
            if (seriesOrder == SeriesRenderingOrder.REVERSE) {
                ...
                for (int pass = 0; pass < passCount; pass++) {
                    ...
                    for (int series = seriesCount - 1; series >= 0; series--) {
                        ...
                        // TODO Bug: recalculate item count for real-time processing when data is updated frequently
                        for (int item = firstItem; item <= lastItem; item++) {
                            int lastItemLocal = dataset.getItemCount(series) - 1;

                            if (lastItemLocal < lastItem) {
                                for (int itemLocal = item; itemLocal <= lastItemLocal; itemLocal++) {
                                    renderer.drawItem(g2, state, dataArea, info,
                                                      this, xAxis, yAxis, dataset, series, itemLocal,
                                                      crosshairState, pass);
                                }
                            } else {
                                renderer.drawItem(g2, state, dataArea, info,
