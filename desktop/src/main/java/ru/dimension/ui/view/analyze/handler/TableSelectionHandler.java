...
public class TableSelectionHandler extends MouseListenerImpl implements ListSelectionListener, KeyListener {
  ...
  public void initializeTableModelListener(int nameId, int checkBoxId) {
    DefaultTableModel model = (DefaultTableModel) jxTableCase.getJxTable().getModel();
    model.addTableModelListener(e -> {
      if (e.getType() == TableModelEvent.UPDATE && e.getColumn() == checkBoxId) { // Checkbox column
        int row = e.getFirstRow();
        try {
          String name = (String) jxTableCase.getJxTable().getValueAt(row, nameId);
          runAction(name);
